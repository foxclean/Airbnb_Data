        ...
        while page < iterations: #<--- Iteración de las páginas.
            ...
            while (allow == False):
                ...
                if len(price) == 0: #<--- Si el tamaño de la lista es 0, quiere decir que hay ningun precio, por ende hay un error y se debe iterar.
                    ...
                    price = get_content(".search-results ._f21qs6 ._v72lrv ._15ns6vh div ._1asxs4e span ._hylizj6 span", filters)
                    #---
            ...
            if (PRICE_STATE == True): #<-- Condición, en caso que PRICE_STATE sea TRUE, como resultado de la extracción de precios, se realiza la siguiente tarea.
                ...
                while (allow == False):
                    ...
                    #---
                        ...
                        filters = get_innerHTML(pagURL)
                        description = get_content(".search-results ._f21qs6 ._v72lrv ._15ns6vh div ._saba1yg ._1127fdt6 span", filters)
                        allow = False
                        DESCRIPTION_STATE = False
                        #---
                    ...
                    while i < (len(clean_desc)): 
                        if (len(clean_desc[i]) == 2):
                            ...
                            if ('cama' in clean_desc[i][0]):
                                ...
                                #num = (re.findall('\\d+', clean_desc[i])) #<--- Se extrae el numero de camas de la descripción.
                                print(clean_desc[i][0])
                                num = (re.findall('\\d+', clean_desc[i][0]))                        
                                bed.append(int(num[0]))
                            else:
                                kind.append(clean_desc[i][0]) #<--- Se extraen el tipo de hospedaje de la descripción.
                                #---
                    ...
                    if (NAME_STATE == True): #<-- Condición, en caso que NAME_STATE sea TRUE, como resultado de la extracción de nombres, se realiza la siguiente tarea.
                        ...
                        while (allow == False):
                            ...
                            if len(r_rating) == 0: #<--- Si el tamaño de la lista de nombres obtenidos es 0.
                                filters = get_innerHTML(pagURL) #<--- Variable donde se manejaran los datos filtrados
                                r_rating = get_content(".search-results ._f21qs6 ._v72lrv ._15ns6vh div div span ._36rlri", filters) #<--- se filtra nuevamente para obtener el nombre.
                                RATE_STATE = False
                                allow = False
                                #---
                                insert_log((CONSULTA[c][0]),("Intento No." + str(attempts) + " - El tamaño de la lista no es el requerido."),'','414',pagURL,2) #tipo 0= error, 1= bien, 2= advertencia
                            ...
                            #---
                                ...
                                temporal_rate = []
                                a = 0
                                for rate in filters.select(".search-results ._f21qs6 ._v72lrv ._15ns6vh div div span ._36rlri ._1uyixqdu span"):
